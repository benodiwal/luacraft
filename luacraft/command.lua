...
-- Create a command. This function is called by Luacraft.
...
-- @tparam Luacraft luacraft CommandOptions
-- @treturn Command
function Command.new(command, config, luacraft)
    config = config or {}
    config.arguments = config.arguments or {}
    assert(luacraft,"Luacraft instance is required")
...
-- @usage
-- cmd:action(
--    function(parsed, command, luacraft)
--      -- command logic
--      -- parse is Parsed
--      -- command is command itself
--      -- luacraft is luacraft instance
...
function Command:set_alias(alias)
    assert(type(alias) == "table" or type(alias) == "string", "Alias should be a table (like-array) or a string")
...
-- @tparam string luacraft_tag Luacraft tag.
function Command:usage(flag_usage, luacraft_tag)
    if(self.hide) then return end
    local cmd_prev = ""
    if(flag_usage)then cmd_prev = self.luacraft.theme.command.category.color("Usage:") end
    cmd_prev = cmd_prev .. "  "
    print(cmd_prev..self.luacraft.theme.command.definition.color((luacraft_tag and luacraft_tag .." " or "") ..self:usage_cmd()).." => " .. self.luacraft.theme.command.description.color(self.description))
end

-- Print Command usage extended
-- @tparam string luacraft_tag Luacraft tag.
function Command:usage_extended(luacraft_tag)
    local usage = self.luacraft.theme.command.category.color("Usage: ").. self.luacraft.theme.command.definition.color((luacraft_tag and luacraft_tag .." " or "") ..self:usage_cmd()) --.." => " .. self.description .. "\n"
    usage = usage .. self.luacraft.theme.command.category.color("\nName: ") .. self.name
    if(#self.alias > 0) then usage = usage .. self.luacraft.theme.command.category.color("; alias: ") .. ftable.join(self.alias, ", ") end
    usage = usage .. "\n"
    if(#self.description > 0) then usage = usage.. self.luacraft.theme.command.category.color("Description: ") .. self.description .. "\n" end 
    if (#self.arguments > 0) then
        usage = usage .. self.luacraft.theme.command.category.color("Arguments:\n")
        for _, argument in ipairs(self.arguments) do
            usage = usage .. "  " .. argument:render_extended() .. "\n"
        end
    end
    if (#self.options > 0) then
        usage = usage .. self.luacraft.theme.command.category.color("Options:\n")
...
-- Check parsed table when add a command as default to Luacraft
...
function Command:check_parsed(parsed)
    ...
    if(not(#required == 0))then self.luacraft:error(err) end
    return (#required == 0)
end
...
function Command.argument_creator(argument, description, type, default)
    ...
    return setmetatable(option, {
        __index = {
            ...
            render_extended = function(t)
                return self.luacraft.theme.command.option.color(t:render()) .. " => " .. self.luacraft.theme.command.description.color(t.description .. (not (t.default == nil) and " (Default: " .. tostring(t.default) .. ")" or ""))
            end,
        }
    })
end
...
function Command:argument_creator(options)
    ...
    return setmetatable(argument, {
        __index = {
            ...
            render_extended = function(t)
                return self.luacraft.theme.command.argument.color(t:render()) .. self.luacraft.theme.command.description.color(t.description and " " .. t.description or "") ..self.luacraft.theme.command.description.color(t.default and " (Default: " .. t.default .. ")" or "")
            end,
        }
    })
end
